...
\subsubsection{Dinámica de la burbuja}
...
\subsubsection{Presión del gas y ecuación de estado}
...
\begin{itemize}
  ...
  \item \textcolor{red}{Rta de Fabián: me tengo que fijar cuál usó Gabriela en su código y usar esa}
...
\ptitle{El código propuesto en la tesis de Gabriela resuelve el problema usando un RK45}
\begin{itemize}
  \item Mencionar que la solución numérica fue implementada en C++ en la tesis de Gabriela en Borland...
  \item Su problema es más gral. Ella trabajó en sonoluminiscencia y cavitación en burbujas haciendo un análisis dinámico y de estabilidad
  \item Como método de evolución temporal empleó un RK45 \textcolor{red}{Referencia para no explicarlo}
  \item Le tomaba correr aproximadamente 10 min \textcolor{red}{Menciono qué compu usé para correrlo?}
\end{itemize}

\ptitle{Se intentó emplear su código. Hubo problemas}
\begin{itemize}
  \item Mencionar que no corre con los compiladores actuales públicos (g++)
  \item Se intentó implementar una versión que corra con los nuevos compiladores pero no fue satisfactorio. Hubo problemas relacionados con error handling internos de Borland que no están directamente implementados en los compiladores públicos.
  \item Se creó un nuevo código en C++ que además de tardar más tiempo en correr (aproximadamente días) no convergía. Se intentó corregirlo pero no se logró.
  \item Se empleó el mismo método numérico que Gabriela
  \item Se optó por usar el clúster de Mecom, lo cual mejoraba el tiempo de cómputo pero no se pudo resolver el problema de la convergencia
\end{itemize}


\ptitle{Este problema pertenece a la categoría de problemas stiff}
\begin{itemize}
  \item Por qué?
  \item Mencionar que es un problema del tipo stiff. Explicar brevente de qué se trata esto
  \item Referenciar algún libro
\end{itemize}

\ptitle{Métodos numéricos para problemas stiff}
\begin{itemize}
  \item Existen métodos numéricos para problemas stiff
  \item Tal es el caso del método Radau \textcolor{red}{Referenciar}
  \item Explicar brevemente
\end{itemize}
...
\begin{itemize}
  ...
  \item Se implementó en python. Se usó la librería scipy.
  \item Comentar los tiempos característicos
  \item Aún falta avanzar agregando más fenómenos físicos
\end{itemize}
